...
namespace DifferentialCollections
{
    ...
    {
        static DifferentialCollectionViewSource(){
            DifferentialDataModel<TKey, TEntity, TCriteria>.PlatformRequeryWithCriteria = (instance, mutator) =>
            {
                ...
                for (int i = 0; i < fields.Length; i++)
                    fields[i].SetValue(copy, fields[i].GetValue(instance.Criteria));

                mutator(copy);
                instance.OnDataSourceChanged(copy);
            };
        }
